...
export default class I18nTransform {
  ast: ASTNode
  options: object
  constructor(options = {}) {
  ...
  genElement(node: ASTNode): string {
    ...
    } else if (node.type === TYPE.STATIC_TEXT) {
      return this.genI18nText(node)
    } else {
      return ''
    }
  }
  ...
  genChildren(node: ASTNode): string {
    ...
    return node.children
      .map((child: ASTNode) => this.genElement(child as ASTNode))
      .filter(isNotEmpty)
      .join('')
  }
  ...
  genI18nText(node: ASTNode): string {
    const { text = '' } = node
